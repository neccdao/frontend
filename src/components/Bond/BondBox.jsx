...
export const BondBox = (props) => {
  ...
  const redemptionFacet = getContract(CHAIN_ID, "NDOLBond");
  ...
  const fixedETHPrice = "350000000000000000000000000000000"
  ...
  useEffect(() => {
    const updateSwapAmounts = () => {
      if (anchorOnFromAmount) {
        ...
        if (isExit) {
          if (fromToken.isNdol) {
            const exitValue = bigNumberify(fromAmount).mul("400000000000000000000000000000").div(fixedETHPrice)
            setToValue(formatAmount(exitValue, 19, 9, false));
            return;
          } else {
